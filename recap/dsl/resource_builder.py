...
class ResourceTemplateBuilder:
    def __init__(self, session: Session, name: str,
        ...
        self._template: ResourceTemplate = ResourceTemplate(name=name, 
    ...
    def add_prop(self, group_name:str, prop_name: str, value_type:str, unit: str,
                 default: Optional[str] = None, create_group=False) -> "ResourceTemplateBuilder":
        prop_group = self.session.execute(select(AttributeTemplate).filter_by(name=group_name)).scalar_one_or_none()
        ...
        prop_value = self.session.execute(select(AttributeValueTemplate)
                                          .filter_by(name=prop_name, value_type=value_type,
                                                     attribute_template=prop_group)
                                      ).scalar_one_or_none()
        if prop_value is not None:
            warnings.warn(f"Property {prop_name} already exists for {group_name}", stacklevel=2)
        else:
            prop_value = AttributeValueTemplate(name=prop_name,
    ...
    def remove_prop(self, group_name: str, prop_name: str) -> "ResourceTemplateBuilder":
        prop_group = self.session.execute(select(AttributeTemplate).filter_by(name=group_name)
                                          .where(AttributeTemplate.resource_templates.
                                                 any(ResourceTemplate.id == self._template.id))
        ).scalar_one_or_none()
    ...
    def add_child(self, name: str, type_names: list[str]):
        child_builder = ResourceTemplateBuilder(self.session,
                                                name=name,
