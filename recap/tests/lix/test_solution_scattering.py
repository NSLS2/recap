...
def test_solution_scattering_sample_prep(db_session):
    ...
    sample_plate_96_well = ResourceTemplate(
        name="96-well sample template", ref_name="96WST", types=[container_resource_type]
    )
    ...
    assert sample_plate_96_well.attribute_templates[0].value_templates[0].default_value == '8'

    # Well attributes
    sample_name = AttributeValueTemplate(name="sample_name", value_type="str", default_value="")
    buffer_name = AttributeValueTemplate(name="buffer_name", value_type="str", default_value="")
    volume = AttributeValueTemplate(name="volume", value_type="int", default_value=0)
    mixing_instruction = AttributeValueTemplate(name="mixing", value_type="str", default_value="")
    stock = AttributeValueTemplate(name="stock", value_type="bool", default_value="False")
    notes = AttributeValueTemplate(name="notes", value_type="str", default_value="")
    well_data = AttributeTemplate(name="well_data",
                                  value_templates=[sample_name,
                                                   buffer_name, volume,
                                                   mixing_instruction, stock,
                                                   notes])
    ...
    for well_name in well_names:
        well_resource_template = ResourceTemplate(name=well_name, ref_name=well_name, types=[well_resource_type])
    ...
    # Well attributes
    sample_well_data = AttributeTemplate(name="sample_holder_well_data",
                                         value_templates=[
                                             sample_name,
                                             buffer_name,
                                             volume
    ...
    for well_num in range(1,19):
        well_resource_template = ResourceTemplate(name=str(well_num), ref_name=str(well_num), types=[well_resource_type])
    ...
    # Create process
    sample_prep_process = ProcessTemplate(name="Solution Scattering Sample Prep", version="1.0")
    sample_plate_resource_slot = ResourceSlot(name="sample_plate",
                                              process_template=sample_prep_process,
                                              resource_type=container_resource_type,
                                          direction=Direction.input)
    holder_resource_slot = ResourceSlot(name="holder",
                                        process_template=sample_prep_process,
                                        resource_type=container_resource_type,
    ...
    volume_attr_value = AttributeValueTemplate(name="volume", ref_name="volume",
                                               value_type="float", unit="uL",
                                               default_value="0")
    volume_transferred_template = AttributeTemplate(name="volume_transferred",
                                                    ref_name="volume_transferred",
                                                    value_templates=[volume_attr_value])

    # Transfer action
    robot_transfer_action_template = StepTemplate(name="Robot transfer",
                                                  process_template=sample_prep_process,
    ...
    robot_transfer_action_template.resource_slots["source_container"] = sample_plate_resource_slot
    robot_transfer_action_template.resource_slots["destination_container"] = holder_resource_slot
    db_session.add(robot_transfer_action_template)
    db_session.commit()

    assert robot_transfer_action_template.resource_slots["source_container"].name == "sample_plate" 
    ...
    db_session.commit()
    process_run1 = ProcessRun(name="Test1", description="This is a test",
