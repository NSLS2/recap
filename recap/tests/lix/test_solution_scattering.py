...
def test_solution_scattering_sample_prep(db_session):
    ...
    num_rows_attr_val = AttributeValueTemplate(name="rows", value_type="int", default_value=8)
    num_cols_attr_val = AttributeValueTemplate(name="cols", value_type="int", default_value=12)
    ...
    well_data = AttributeTemplate(
        name="well_data", value_templates=[sample_name, buffer_name, volume, mixing_instruction, stock, notes]
    )
    ...
    sample_well_data = AttributeTemplate(
        name="sample_holder_well_data", value_templates=[sample_name, buffer_name, volume]
    )
    ...
    volume_attr_value = AttributeValueTemplate(
        name="volume", ref_name="volume", value_type="float", unit="uL", default_value="0"
    )
    volume_transferred_template = AttributeTemplate(
        name="volume_transferred", ref_name="volume_transferred", value_templates=[volume_attr_value]
    )
    ...
    db_session.commit()
    process_run1 = ProcessRun(name="Test1", description="This is a test", template=sample_prep_process)
