...
class ProcessTemplate(Base):
    ...
    step_templates: Mapped[List["StepTemplate"]] = relationship(back_populates="process_template")
    edges: Mapped["StepTemplateEdge"] = relationship(
        "StepTemplateEdge",
        primaryjoin=id == StepTemplateEdge.process_template_id,
        cascade="all, delete-orphan",
    )
    resource_slots: Mapped[List["ResourceSlot"]] = relationship("ResourceSlot", back_populates="process_template")
    __table_args__ = (UniqueConstraint("name", "version", name="uq_process_template_name_version"),)
...
class ResourceSlot(Base):
    ...
    process_template_id: Mapped[UUID] = mapped_column(ForeignKey("process_template.id"), nullable=False)
    process_template: Mapped[ProcessTemplate] = relationship(ProcessTemplate, back_populates="resource_slots")
    resource_type_id: Mapped[UUID] = mapped_column(ForeignKey("resource_type.id"), nullable=False)
    ...
    __table_args__ = (UniqueConstraint("process_template_id", "name", name="uq_process_template_name"),)
...
class ProcessRun(Base):
    ...
    resources = associationproxy.association_proxy(
        "assignments",
        "resource",
        creator=lambda res_slot, resource: ResourceAssignment(resource_slot=res_slot, resource=resource),
    )
    ...
    def _check_resource(self, key, resource):
        ...
        acceptable_slots = {
            slot.id for slot in self.template.resource_slots if slot.resource_type_id == resource.resource_type_id
        }
        ...
        if resource.resource_slot_id in used_slots:
            raise ValueError(f"Slot {resource.resource_slot_id} is already occupied in run {self.id}")
...
class ResourceAssignment(Base):
    ...
    process_run_id: Mapped[UUID] = mapped_column(ForeignKey("process_run.id"), primary_key=True)
    resource_slot_id: Mapped[UUID] = mapped_column(ForeignKey("resource_slot.id"), primary_key=True)
    resource_id: Mapped[UUID] = mapped_column(ForeignKey("resource.id"), nullable=False)

    # ties back to the run
    process_run: Mapped["ProcessRun"] = relationship("ProcessRun")  # , back_populates="assignments"
    ...
    __table_args__ = (UniqueConstraint("process_run_id", "resource_slot_id", name="uq_run_slot"),)
